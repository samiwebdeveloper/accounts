...
class Invoicemodel extends CI_Model
{
	...
	{
		return $this->db->query("SELECT * FROM `acc_invoice_detail` WHERE `invoice_id` = $invoice_id AND cn not in (SELECT crn_cn FROM `cr_note` n join cr_note_detail d on d.crn_id = n.crn_id where n.invoice_id = $invoice_id)")->result();
		// return $this->db->query("SELECT * FROM `acc_invoice_detail` WHERE `invoice_id`=$invoice_id")->result();
	}
	public function get_invoice_detail($invoice_no)
	{
		return  $this->db->query("SELECT * FROM `acc_invoice` WHERE invoice_code='$invoice_no'")->row();
	}
	...
	{
		return  $this->db->query("SELECT `acc_invoice_id` FROM `acc_invoice` WHERE invoice_code='$invoice_no'")->row()->acc_invoice_id;
	}
	...
	{
		return  $this->db->query("SELECT reference_by from acc_customers where customer_id =$customer_id")->row()->reference_by;
	}
	public function outstanding_amount($customer_id)
	{
	...
	{
		$query = "SELECT `acc_customers`.`customer_id` as `id`,`acc_customers`.`customer_name` as `name`,`city_name` as `city`,is_gst, account_type
		FROM `acc_orders`
	...
	{
		$query = "SELECT `customer_name`, `is_gst`, `gst`, `account_type` FROM `acc_customers` WHERE `customer_id`=?";
	...
	public function Get_CN_BY_Customer_ID_And_Date($customer_id, $date, $date_f, $destination, $service)
	{
		...
		JOIN `acc_services` on `acc_services`.`service_id` = `acc_orders`.`order_service_type`
		WHERE `is_invoice`= 0 
		AND `customer_id`= ? 
	...
	public function Get_CN_BY_Franchisee_ID_And_Date($franchisee_id, $date, $date_f, $destination, $service)
	{
		$query = "SELECT * FROM `acc_orders` 
		JOIN `acc_services` on `acc_services`.`service_id` = `acc_orders`.`order_service_type`
		WHERE `is_invoice`= 0 
		...
		$query .= strlen($service) > 0 ? " AND `order_service_type` IN (" . $service . ")" : "";
		$query .= " ORDER BY `order_date`;";
		$res = $this->db->query($query, array($franchisee_id, $date, $date_f));
		return $res->result();
	}
	...
	public function get_unbilled_cn($booking_date_1, $booking_date_2)
	{
		$query = "SELECT * FROM `unbilled_cn`
			where order_date between '" . $booking_date_1 . "' and '" . $booking_date_2 . "'";
		$res = $this->db->query($query);
		return $res->result_array();
	}
}
