...
class Bookingmodel extends CI_Model
{
	/*public function Get_Active_Pickup_Points_By_Customer_id($cid){
		...
		public function get_cash_collection_balance($order_id)
		{
			return  $this->db->query("SELECT SUM(amount) as balance FROM `cash_collection` WHERE order_id='$order_id' group by order_id")->row();
		}

	public function Get_cash_booking_report($startdate, $enddate, $o_city)
	{
		if ($o_city == "") {
			$query = "SELECT
			o.`order_id`,
		...
		FROM
			acc_orders o
		JOIN cash_collection c ON
			o.order_id = c.order_id
			LEFT JOIN cargo.`saimtech_oper_user` ops ON
			 c.created_by = ops.oper_user_id 
		LEFT JOIN cn_book_usage b ON
			 b.manual_cn = o.manual_cn 
		LEFT JOIN cn_issue i ON
						i.book_id = b.book_id
		LEFT JOIN `cargo`.`saimtech_rider` r ON
						r.rider_id = i.issue_to
		LEFT JOIN `cargo`.`saimtech_route` u ON
						u.route_id = i.route
		WHERE
			o.`order_pay_mode` in ('cash','Cash') 
	...
	public function Get_order_Data_By_Date_Range($startdate, $enddate, $o_city)
	{
		if ($o_city == "") {
			$query = "SELECT
			o.`acc_orders_id`,
		...
		FROM
			acc_orders o
		LEFT JOIN cash_collection c ON
			o.order_id = c.order_id
		LEFT JOIN cn_book_usage b ON
			b.manual_cn = o.manual_cn
		LEFT JOIN cn_issue i ON
			i.book_id = b.book_id
		LEFT JOIN `cargo`.`saimtech_rider` r
		ON
			r.rider_id = i.issue_to
		LEFT JOIN `cargo`.`saimtech_route` u
		ON
			u.route_id = i.route
		WHERE
			o.`order_pay_mode` IN('cash', 'Cash') AND DATE(order_date) BETWEEN '$startdate' AND '$enddate' 
		GROUP BY
			o.order_id
		ORDER BY
			order_id";
		...
		ON
			u.route_id = i.route
		WHERE
			o.`order_pay_mode` IN('cash', 'Cash') AND DATE(order_date) BETWEEN '$startdate' AND '$enddate' AND origin_city = '$o_city'
		GROUP BY
			o.order_id
		ORDER BY
			order_id;";
	...
	{
		$query = "SELECT DATE(`order_date`) AS `order_date`, `origin_city_name`, `order_pay_mode`, FORMAT(COUNT(DISTINCT `order_id`), 0) AS `order_count` FROM `acc_orders` 
			WHERE `is_hardchecked` = 0 AND `is_invoice` = 0 AND `origin_city` IN (SELECT `city_id` FROM `acc_city` WHERE `mixture` IN ('" . str_replace(",", "','", $_SESSION['user_mixture']) . "'))
			 GROUP BY DATE(`order_date`), `origin_city_name`, `order_pay_mode` ORDER BY DATE(`order_date`) DESC, `origin_city_name`, `order_pay_mode` ";
		$res = $this->db->query($query);
		return $res->result_array();
	}
	...
	{
		$query = "SELECT DATE(`order_date`) AS `order_date`, `origin_city_name`, `order_pay_mode`, FORMAT(COUNT(DISTINCT `order_id`), 0) AS `order_count` FROM `acc_orders` 
			WHERE `is_hardchecked` = 1 AND `is_invoice` = 0 AND `origin_city` IN (SELECT `city_id` FROM `acc_city` WHERE `mixture` IN ('" . str_replace(",", "','", $_SESSION['user_mixture']) . "')) 
			GROUP BY DATE(`order_date`), `origin_city_name`, `order_pay_mode` ORDER BY DATE(`order_date`) DESC, `origin_city_name`, `order_pay_mode` ";
		$res = $this->db->query($query);
		return $res->result_array();
	}
