...
class Rider extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($_SESSION['user_power'] != "TPT") {
			$data['rider_data'] = $this->Commonmodel->Get_record_by_double_condition('cargo.saimtech_rider', 'rider_origin_id', $_SESSION['origin_id'], 'agent_id', 0);
		} else {
			$data['rider_data'] = $this->Commonmodel->Get_record_by_condition('cargo.saimtech_rider', 'agent_id', $_SESSION['thrid_party_id']);
		}
		...
		if ($rider_name != "" && $rider_cnic != "") {
			...
			if ($check == 0) {
				$data = array(
					...
					'rider_empcode'             => $rider_empcode,
					'is_enable'                 => 1,
					'agent_id'                  => $_SESSION['thrid_party_id'],
					'created_by'                => $_SESSION['user_id'],
					'created_date'              => date('Y-m-d H:i:s')
				);
				$rider_id = $this->Commonmodel->Insert_record('cargo.saimtech_rider', $data);
	...
	public function Rider_code($rider_id)
	{
		...
		if (strlen($rider_id) == 1) {
			$precode = $prefix . "0000" . $code;
		} else if (strlen($rider_id) == 2) {
			$precode = $prefix . "000" . $code;
		} else if (strlen($rider_id) == 3) {
			$precode = $prefix . "00" . $code;
		} else if (strlen($rider_id) == 4) {
