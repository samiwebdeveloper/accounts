<?php
	...
	class Booking extends CI_Controller {
		...
		public function check_manual_cn(){
			$manual_cn= $this->input->post('order_code'); 
			if($manual_cn!=""){
	...
	public function edit_booking_view()
	{
		...
		public function select(){
			$this->load->view('module_booking/selectbookingView');
		}	
		...
		public function add_shipment(){
			...
			} else {$message = "<div class='pgn push-on-sidebar-open pgn-bar'><div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'><span aria-hidden='true'>×</span><span class='sr-only'>Close</span></button><strong>Duplicate Manual CN Error !</strong></div></div>";}
			//=========================================================END
			echo $message;	
		}		
		...
		public function update_rates(){
			...
			$o_fuel = ceil($o_sc * $c->fuel_rate);
			$o_oth = ceil($o_sc * $c->others_rate);
			$o_gst = ceil(($o_sc + $o_fuel + $o_oth) * $c->gst_rate);
			$o_tot = ceil($o_sc + $o_gst + $o_fuel + $o_oth);					
